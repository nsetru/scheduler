...
function scheduler_prepare_formdata($scheduler, $slot) {
    ...
    // -- ucl hack begin --
    //https://svn.ucl.ac.uk/projects/isd/moodle26/ticket/28 Scheduler Plugin - get 'caneditlocation' field from mdl_local_scheduler_extension
    require_once(__DIR__.'/../../local/scheduler_extension/lib.php');
    $data->caneditlocation = scheduler_extension_getdata($scheduler->id, $slot->id);
    // -- ucl hack end --
...
function scheduler_save_slotform($scheduler, $course, $slotid, $data) {
    ...
    // -- ucl hack begin --
    //https://svn.ucl.ac.uk/projects/isd/moodle26/ticket/28 Scheduler Plugin - populate mdl_local_scheduler_extension

    $caneditlocation = (isset($data->caneditlocation) ? $data->caneditlocation : 0);
    require_once(__DIR__.'/../../local/scheduler_extension/lib.php');
    scheduler_extension_saveformdata($scheduler->id, $slot->id, $caneditlocation);
    //-- ucl hack end --
