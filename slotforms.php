...
abstract class scheduler_slotform_base extends moodleform {
    ...
    protected function add_base_fields() {
        ...
        // -- ucl hack begin --
        //https://svn.ucl.ac.uk/projects/isd/moodle26/ticket/28 Scheduler Plugin - students enabled to enter location
        //Add a new checkbox 'caneditlocation' that would allow tutor/course admin to set, whether students are allowed to edit location
        ...
        $locationgroup=array();
        $locationgroup[] = $mform->createElement('text', 'appointmentlocation', get_string('location', 'scheduler'), array('size'=>'30'));
        $mform->setType('appointmentlocation', PARAM_NOTAGS);
        $mform->setDefault('appointmentlocation', scheduler_get_last_location($this->scheduler));
        
        $locationgroup[] = $mform->createElement('checkbox', 'caneditlocation', '',get_string('enablecaneditlocation', 'local_scheduler_extension'));
        $locationgroup[] = $mform->createElement('static', 'caneditlocationlabel', get_string('enablecaneditlocation', 'local_scheduler_extension'));
        $mform->addGroup($locationgroup, 'locationgroup', get_string('location', 'scheduler') , ' ', false);
        $mform->addGroupRule('locationgroup', array('appointmentlocation' => array(array(get_string('maximumchars', '', 50), 'maxlength', 50))));
        $mform->addHelpButton('locationgroup', 'location', 'scheduler');
        // -- ucl hack end --
